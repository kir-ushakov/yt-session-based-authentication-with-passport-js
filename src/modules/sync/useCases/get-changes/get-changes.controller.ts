...
import { IGetChangesRequestDTO, IGetChangesResponseDTO } from "./get-changes.dto";
...
export class GetChnagesController extends BaseController {
  ...
  protected async executeImpl(req: Request, res: Response, next?: NextFunction): Promise<Response> {
    ...
    try {
      ...
      if(result.isFailure) {
        ...
        switch (result.constructor) {
          case GetChangesErrors.ClientNotFoundError : 
            const errorDto: IApiErrorDto = {
              type: EApiErrorType.INVALID_CLIENT_ID,
        ...
        const changesDTO: IGetChangesResponseDTO = { changes : changes.map(change => ChangeMapper.toDTO(change)) };
