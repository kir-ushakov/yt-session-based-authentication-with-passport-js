...
import { IVerificationTokenProps, VerificationToken } from "../domain/values/user/verification-token"
...
export class UserRepo {
  ...
  public async exist(userEmail: UserEmail): Promise<boolean> {
    ...
    const existingEmail = await UserModel.findOne({ username : userEmail.value });
  ...
  public async getTokenByTokenId(tokenId: string): Promise<VerificationToken> {
    ...
    const tokenDocument: VerificationTokenDocumnet = await VerificationTokenModel.findOne({ token : tokenId });
  ...
  public async save(user: User): Promise<UserDocument> {
    ...
    const updatedUser: UserDocument = await UserModel.findOneAndUpdate(filter, update, { useFindAndModify: false });
